    ...
    def main():
        def banner():
            print(p+"""
                                              ...
                                              """+g+""""""+reset)
            print("           "+y+"<========[[ "+s_b+"INSTAGRAM "+blink+"RECON"+reset+y+" ]]========>\n"+reset)
            print("        "+y+"<--------( "+r+"Coded by : DeviCon Team"+y+" )-------->\n"+reset)

        # recon data
        def recon():
            global json_data, ReportList
            print(reset)
            req = requests.Session()
            res = req.get(url="https://instagram.com/{}/?__a=1&__d=dis".format(username))
            json_data = res.json()
            ...
            count = 1
            print(pi+"["+y+"00"+pi+"]"+s_b+" profile link : "+g+"https://instagram.com/{}/".format(username),reset)
            for i in ReportList:
        ...
        # report saving
        def save_report():
            if not os.path.exists("{}".format(username)):
            ...
            with open("{}/{}.txt".format(username,username),'w') as f:
                f.write(json_report)

            response = requests.get(pic_url) # download profile image
            if response.status_code:
                with open("{}/{}_profile_pic.jpg".format(username,username), 'wb') as img:
