...
def time_start():
    ...
    print(Lyellow+"Time started: {}".format(current_time1)+reset)

def time_finish():
    time_now2=datetime.datetime.now()
    current_time2 = time_now2.strftime("%H:%M:%S")
    print(Lyellow+"Time Finished: {}".format(current_time2)+reset)
...
def general_udp_scanning(ip):
    ...
    print('[name]: ' + LightB + result['scan'][ip]['hostnames'][0]['name'] + reset + '\n[operating system]: '+ LightB+os_scan['scan'][ip]['osmatch'][0]['name'] + reset
...
def UDP_Scanning(ip):
    while True:
        print(Lcyan+bold+"═" * 25 + " Please choose a number " + "═" * 25+reset)
        choice = input(Lmagenta+
       ...
       #scan specific ports
        if choice == '1':
            ...
            time_start()
            while True:
                print(Lyellow+"Please enter the range of ports you want to scan in format: <int>-<int> (ex: 60-120)"+reset)
